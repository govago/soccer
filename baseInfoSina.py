...
class BasePageDetail(PageDealer):
    ...
    def pick(self,request):
        ...
        try:
            br.visit(self._url)
            #using 20 for zhuJinQiShuLiang & keJinQiShuLiang
            #br.find_by_id('zhuJinQiShuLiang').first.select("20")
            #br.find_by_id('keJinQiShuLiang').first.select("20")
            retryTimes = 0
        ...
        if body.find('table') == -1:
            ...
            self._foExcept.write('%s\t%s\n' % (self._round, self._url))
            return []
        zhuTotalText = br.find_by_id('zhuJinQiTotal').first
        zhuText = zhuTotalText.text
        keTotalText = br.find_by_id('keJinQiTotal').first
        keText = keTotalText.text
        page = BeautifulSoup(body, 'html5lib')
        goodBody = page.find('body', id='sortable')
        tables = []
        tables.append(goodBody.find('tbody', id='jiaozhanBody'))
        tables.append(goodBody.find('tbody', id='zhuJinQi'))
        tables.append(goodBody.find('tbody', id='keJinQi'))
        tables.append(zhuText)
        tables.append(keText)  
        pdb.set_trace()
        return tables
...
UrlPattern = 'http://odds.sports.sina.com.cn/liveodds/base/?m_id='
def yieldPage(config):
    rounds = config['round_interval'].strip().split('-')
    for bid, mid in sorted(bidMidMap.items(), \
            key = lambda en : en[0], reverse=True):
